        ...
        <div class="table_container">
            <el-row style="margin-bottom: 30px;">
                <div style="display: flex;align-items: center;justify-content: flex-end;margin-bottom: 5px">
                    ...
                    <el-button style='' type="primary" icon="document" @click="handleDownload" :loading="downloadLoading"> 导出excel</el-button></div>
            ...
            <el-table
                ...
                <el-table-column label="应用图标" prop="fileIcon" min-width="50">
                    <template scope="scope">
                      <img :src="'https://imapp-image.oss-cn-hangzhou.aliyuncs.com/'+scope.row.fileIcon" style="width:30px;height:30px;">
                    </template>
                ...
                <el-table-column
                    ...
                    min-width="50">
                    <template scope="scope">
                        <router-link :to="{path:'/appInfo',query:{id:scope.row.fileId,activeName:activeName,type:'android'}}">{{scope.row.appName}}</router-link>
                    </template>
                </el-table-column>
                <!--<el-table-column-->
                    <!--label="应用类型"-->
                ...
                <el-table-column label="操作" min-width="50">
                    <template scope="scope">
                        ...
                        <!--</el-button>-->
                        <el-button class="littleButton" @click="update(scope.row.fileId)"   v-if="activeName==1" key="edit">编辑</el-button>
                        <el-button  class="littleButton" v-if="activeName==1" key="update" @click="updateMore(scope.row.fileId)">更新</el-button>
                        <el-button  class="littleButton"  @click="deletes(scope.row.fileId)" v-if="activeName==0||scope.row.fileState==0">删除</el-button>
                        <el-button class="littleButton" @click="check(scope.row.fileId)"  v-if="activeName==0" key="uncheck">审核
                        </el-button>
                    </template>
                </el-table-column>
            </el-table>
...
export default {
  ...
  created() {
    ...
    this.queryListData({activeName:this.activeName,pageValue:this.$route.query.page||1,pageSize:this.$route.query.size||10});
  ...
  methods: {
    queryListData({ activeName, pageValue, pageSize }) {
      this.$ajax
        .get(`${BaseUrl}apply/all`,{params:{pageCode:pageValue || 1,pageSize:pageSize || 10,state:activeName || 0,fileState:this.flag,tagId: this.fileTagUnchoice==-1?null:this.fileTagUnchoice,keyWords:(this.searchInfo!=null&&this.searchInfo!='')?this.searchInfo:null},headers:{'token': sessionStorage.getItem('token'),'device':'android'}})
        .then(response => {
    ...
    update(id) {
      ...
      ensureDelete(){
          ...
          this.$ajax.get(BaseUrl + 'apply/del/' + this.delId,{headers: {'token': sessionStorage.getItem('token'),'device':'android'}}).then(response => {
              this.delId='';
              if(response.data.flag==200){
                  var t;
                  clearTimeout(t)
                  ...
                  that.queryListData({activeName:that.activeName,pageValue:that.currentPage,pageSize:that.nowPageSize});
              }else if(response.data.flag==201) {
                  this.$alert(response.data.msg + '，请重新登录', '提示', {
                      confirmButtonText: '确定',
                      callback: action => {
      ...
      handleDownload() {
          this.$ajax
              .get(`${BaseUrl}apply/all/0/0/${this.activeName}`,{headers:{'token': sessionStorage.getItem('token'),'device':'android'}})
              .then(response => {
                  if(response.data.flag==200){
                  this.info = response.data.data.appList;
                  this.txcount = response.data.data.fileNum;
