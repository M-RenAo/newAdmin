        ...
        <div class="table_container">
            ...
            <el-table
                ...
                <el-table-column
                    ...
                    min-width="50">
                    <template scope="scope">
                        <router-link :to="{path:'/appInfo',query:{id:scope.row.fileId,activeName:activeName,type:'ios'}}">{{scope.row.appName}}</router-link>
                    </template>
                </el-table-column>
                <!--<el-table-column-->
                    <!--label="应用类型"-->
    ...
    export default {
        ...
        methods: {
            ...
            ensureDelete(){
                ...
                this.$ajax.get(BaseUrl + 'apply/del/' + this.delId,{headers: {'token': sessionStorage.getItem('token'),'device':'ios'}}).then(response => {
                    this.delId='';
                    if(response.data.flag==200){
                        var t;
                        clearTimeout(t)
            ...
            handleDownload() {
                this.$ajax
                    .get(`${BaseUrl}apply/all/0/0/${this.activeName}`,{headers: {'token': sessionStorage.getItem('token'),'device':'ios'}})
                    .then(response => {
                        if(response.data.flag==200) {
                            this.info = response.data.data.appList;
                            this.txcount = response.data.data.fileNum;
