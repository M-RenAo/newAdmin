  ...
  methods: {
    ...
    submitUpload() {
        ...
        Promise.all([promise,promise2]).then((result) => {
            ...
            var that=this
            this.$ajax.get(BaseUrl+'apply/auto/'+this.$route.query.id,{headers: {'token':sessionStorage.getItem('token'),'device':that.$route.query.type}}).then(function (response) {
                if(response.data.flag==200){
                    ...
                    that.$alert(response.data.msg, "提示", {
                        ...
                        callback:action=>{
                            ...
                            that.$router.push({path:'/applicationList'})
                            }else if(that.$route.query.type=='ios'){
                                that.$router.push({path:'/iosList'})
                            }
                        }
                    });
                }else if(response.data.flag==201) {
