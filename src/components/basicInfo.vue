<template>
    <div>
        <div style="margin:20px;border: 1px solid #eaeefb;font-size: 14px;color:#1f2d3d">
            <el-row type="flex" class="row-bg" >
                <el-col :span="6">
                    <div style="display: flex;align-items: center;">
                    <span>头像：</span><img :src="'https://imapp-image.oss-cn-hangzhou.aliyuncs.com/'+userForm.headPic" style="height:30px; width:30px;display: inline-block;margin-left:10px;" v-if="userForm.headPic!=undefined">
                </div>
                </el-col>
                <el-col :span="6">
                    <div style="display: flex;align-items: center;">
                    <span>昵称：</span><span>{{userForm.userNickName}}</span>
                   </div>
                </el-col>
                <el-col :span="6">
                    <div style="display: flex;align-items: center;">
                    <span>手机号：</span><span style="display: inline-block;margin-left:10px;">{{userForm.userPhone}}</span>
                    </div>
                </el-col>
                <el-col :span="6">
                    <div style="display: flex;align-items: center;">
                       <span>ID：</span><span style="display: inline-block;margin-left:10px; word-wrap:break-word; min-width:150px;">{{userForm.userId}}</span>
                    </div>
                </el-col>
            </el-row>
            <el-row type="flex" class="row-bg" >
                <el-col :span="6">
                    <div style="display: flex;align-items: center;">
                        <span>姓名：</span><span style="display: inline-block;margin-left:10px;">{{userForm.name}}</span>
                    </div>
                </el-col>
                <el-col :span="12">
                    <div style="display: flex;align-items: center;">
                        <span>身份证号：</span><span>{{userForm.cardId}}</span>
                    </div>
                </el-col>
            </el-row>
            <el-row type="flex" class="row-bg" >
                <el-col :span="12">
                    <div style="display: flex;align-items: center;">
                        <span>最近登录时间：</span><span style="display: inline-block;margin-left:10px;">{{userForm.loginTime}}</span>
                    </div>
                </el-col>
                <el-col :span="12">
                    <div style="display: flex;align-items: center;">
                        <span>第一次登录时间：</span><span style="display: inline-block;margin-left:10px;">{{userForm.firstLoginDate}}</span>
                    </div>
                </el-col>
    ...
    export default {
        ...
        created() {
           ...
           this.userlist.forEach(item=>{
               if(this.$route.query.id==item.userId){
                 this.userForm=item;
               }
           })
        },
