        ...
        <div class="table_container">
            <el-tabs v-model="activeNames" @tab-click="change()">
                ...
                <el-tab-pane label="BCD" name="second">
                   <div v-if="activeNames=='second'"> <el-row type="flex" style="margin-bottom: 20px;">
                        <el-col :span="18" style="display:flex;height: auto;word-break:break-all;flex:1">
                            <div style="display:inline-block">
                                <div style="display: inline-block">
                                    <span style="font-size: 14px;width:80px;">时间：</span>
    ...
    export default {
        ...
        created(){
            ...
            if(this.activeNames=='first'){
                const form={userId:this.$route.query.id,pageCode:1,pageSize:10,type:1,startTime:'',endTime:''}
                this.getData(form);
            }else{
                const form={userId:this.$route.query.id,pageCode:1,pageSize:10,type:0,startTime:'',endTime:''}
                this.getData(form);
            }
        },
        computed: {
        ...
        methods: {
            change(){
                if(this.activeNames=='first'){
                    ...
                    this.txcount=0;
                    this.balance=''
                    this.getData({userId:this.$route.query.id,pageCode:1,pageSize:10,type:1,startTime:'',endTime:''});
                    ...
                    this.txcount=0;
                    this.balance=''
                    this.getData({userId:this.$route.query.id,pageCode:1,pageSize:10,type:0,startTime:'',endTime:''});
                }            },
            getData(form){
                this.$ajax({
                    method: "POST",
